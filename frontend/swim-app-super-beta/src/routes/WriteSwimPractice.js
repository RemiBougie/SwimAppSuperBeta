...
export default function WriteSwimPractice({
  swimPractice_id = null,
  parentOpenModal = null,
  parentCloseModal = null, 
  submitHandler = null,
}) {
  ...
  if (swimPractice_id === "new") {
    ...
    swimPractice = allSwimPractices.find(
      (swimPractice) => swimPractice.id === swimPractice_id
    );
    existing = true;
  }
  ...
  for (const swimSet_id of swimPractice["body"]) {
    let foundSwimSet = allSwimSets.find(
      (swimSet) => swimSet["id"] === swimSet_id
    );
    if (foundSwimSet) {
  ...
  for (const category in allTagsClone) {
    Object.keys(allTagsClone[category]).map((subcategory) => {
      allTagsClone[category][subcategory] =
        swimPractice["swimPractice_tags"].includes(subcategory);
    });
  }
  ...
  // modal setup for swimSet selection
  ...
  const removeSwimSet = (selectedSwimSet) => {
    setSwimSets(
      swimSets.filter((swimSet) => swimSet.id !== selectedSwimSet.id)
    );
  };

  const removeSwimSetOption = [
    {
      onClick: (swimSet) => removeSwimSet(swimSet),
  ...
  const validateData = (saveAsNew) => {
    ...
    try {
      ...
      if (swimSets.length === 0) {
        fieldErrors["swimSets"] = "At least one swim set is required.";
      }
    ...
    if (formData["rating"] < 0.0 || formData["rating"] > 5.0) {
      formErrors.push("Rating must be between 0.0 and 5.0");
    }
  ...
  return (
    <div className="write-swim-practice">
      <label>Swim Practice Title</label>
      <br />
      <input
        type="text"
      ...
      <label>Swim Sets</label>
      <br />
      <div className="writeSwimPractice-displaySwimSets">
        {swimSets.map((item) => {
          //console.log("this swimSet should be displaying...", item);
          //return <SwimSetCard swimSet={item} additionalMenuOptions={removeSwimSetOption} clickHandler={removeSwimSet}/>
          return <SwimSetCard swimSet={item} openModal={openModal} closeModal={closeModal} setModalComponent={setModalComponent} removeSwimSet={removeSwimSet} />;
        })}
      </div>
      ...
      <button
        onClick={() => {
          openModal();
          setModalComponent(<BrowseSwimSets clickHandler={addSwimSet} />);
        }}
      >
        Select Existing Set
      </button>
