...
async function getAllSwimSets() {
  //(setSwimSets, setLoading, setItemList, generateSwimSetCards) {
  if (isMock) {
...
async function postSwimSet(data) {
  ...
  if (isMock) {
    let existingDataIndex = mockSwimSets.findIndex(
      (swimSet) => swimSet["id"] === data["id"]
    );
    //let existingDataIndex = mockSwimSets.find(swimSet => { swimSet["id"] === data["id"]})
    console.log(
      "Matching swimSet_id found at index: ",
      existingDataIndex,
      " of mockSwimSets."
    );
...
async function postSwimPractice(data) {
  if (isMock) {
    ...
    let existingDataIndex = mockSwimPractices.findIndex(
      (swimPractice) => swimPractice["id"] === data["id"]
    );
    //let existingDataIndex = mockSwimSets.find(swimSet => { swimSet["id"] === data["id"]})
    if (existingDataIndex >= 0) {
...
async function postSwimSeason(metadata) {
  if (isMock) {
    let existingDataIndex = mockSwimSeasons.findIndex(
      (swimSeason) => swimSeason["id"] === metadata["id"]
    );
    console.log("existingDataIndex in postSwimSeason: ", existingDataIndex);
    if (existingDataIndex >= 0) {
      let swimSeason = mockSwimSeasons[existingDataIndex];
      swimSeason["title"] = metadata["title"];
      swimSeason["notes"] = metadata["notes"];
      swimSeason["startDate"] = metadata["startDate"];
      swimSeason["endDate"] = metadata["endDate"];
      console.log('new mockSwimSeasons: ', mockSwimSeasons);
    } else {
      let body = [];
      for (var d = metadata["startDate"]; d <= metadata["endDate"]; d.setDate(d.getDate()+1)) {
        body.push({
            "id": v4(),
            "datetime": new Date(d),
            "planned": null,
      ...
      metadata["body"] = body;
      mockSwimSeasons.push(metadata);
      console.log('new mockSwimSeasons: ', mockSwimSeasons);
      //iteratively call postSwimSeasonPlan between startDate and endDate
    }
    // actually, check if it exists. If so, update, otherwise push
  } else {
    throw new Error("Live data not set up yet, switch to mockData");
...
async function deleteSwimSet(data) {
  if (isMock) {
    ...
    let dataIndex = mockSwimSets.findIndex(
      (swimSet) => swimSet["id"] === data["id"]
    );
    if (dataIndex > 0) {
...
async function deleteSwimPractice(data) {
  if (isMock) {
    ...
    let dataIndex = mockSwimPractices.findIndex(
      (swimPractice) => swimPractice["id"] === data["id"]
    );
    if (dataIndex > 0) {
      //console.log("prior to delete: ", mockSwimSets);
